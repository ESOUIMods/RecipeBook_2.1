    ...
    if #RB.BankValueTable<4 then
	    ...
	    for i=1,#RB.BankValueTable do
		    ...
		    _G["RBUI_Row"..i]:SetHandler("OnMouseEnter", function(self)
				...
				ItemTooltip:SetAnchor(CENTER,_G["RBUI_Row"..i.."ButtonIcon"]:GetParent(),CENTER,-600,0)
			...
			if RBDisplayCraft == true and RBcraftcounttotal ~= 0 then
			_G["RBUI_Row"..i.."Name"]:SetText(zo_strformat(SI_TOOLTIP_ITEM_NAME, RB.BankValueTable[i].link).." |c000000x"..tostring(RBcraftcounttotal))
			else
		...
		RBUI_ContainerSlider:SetHidden(true)
		    ...
		    _G["RBUI_Row"..i]:SetHandler("OnMouseEnter", function(self)
				...
				ItemTooltip:ClearAnchors()
					...
					--ItemTooltip:SetAnchor(CENTER,_G["RBUI_Row"..i.."ButtonIcon"]:GetParent(),CENTER,-480,0)
		    	else
				ItemTooltip:ClearAnchors()
		    		ItemTooltip:SetAnchor(CENTER,OldAnchor,CENTER,500,0)
					--ItemTooltip:SetAnchor(CENTER,_G["RBUI_Row"..i.."ButtonIcon"]:GetParent(),CENTER,500,0)
		    	end
			...
			_G["RBUI_Row"..i.."Name"]:SetFont(RB.items.SaveVars.choosenrecipenamefont.f)
			if RBDisplayCraft == true and RBcraftcounttotal ~= 0 then
			_G["RBUI_Row"..i.."Name"]:SetText(zo_strformat(SI_TOOLTIP_ITEM_NAME, RB.BankValueTable[last].link).." |c000000x"..tostring(RBcraftcounttotal))
			else
...
function RB.SavePlayerInvent()--Called on Load/Close To Fill Backpack Data
	...
	for listIndex = 1, lists do
		...
		elseif RBrecipetype ~= RB.SDDI and RB.SDDI == "All" then
			...
			if zo_strformat(SI_TOOLTIP_ITEM_NAME,recipename) ~= "Porky Port" then
				--add to table
...
if (not RB.AddonReady) then return end
		...
		-- if RB.items.SaveVars.ingredientcolor.r == 255 then
	
			-- RB.items.SaveVars.ingredientcolor.r = 0.156863
		-- end
		-- if RB.items.SaveVars.ingredientcolor.g == 255 then
			-- RB.items.SaveVars.ingredientcolor.g = 0.074510
		-- end
		-- if RB.items.SaveVars.ingredientcolor.b == 255 then
			-- RB.items.SaveVars.ingredientcolor.b = 0.019608
		-- end	
		-- if RB.items.SaveVars.ingredientcolor.a == 255 then
			-- RB.items.SaveVars.ingredientcolor.a = 0.950820
		-- end
...
function RBBuildLAMSettingsMenu()
	...
	--done var switch to finished
	RB.LAMDONE = 1	
end
function RB.DoLAMColorPickerHack()
	if RB.items.SaveVars.ingredientcolor.r == 255 then
	RB.items.SaveVars.ingredientcolor.r = 0.180392
	end
	if RB.items.SaveVars.ingredientcolor.g == 255 then
	RB.items.SaveVars.ingredientcolor.g = 0.082353
	end
	if RB.items.SaveVars.ingredientcolor.b == 255 then
	RB.items.SaveVars.ingredientcolor.b = 0.027451
	end
	if RB.items.SaveVars.ingredientcolor.a == 255 then
	RB.items.SaveVars.ingredientcolor.a = 1
	end
end
...
--FIXRESIZE
EVENT_MANAGER:RegisterForEvent("RecipeBook", EVENT_PLAYER_ACTIVATED,
    function()
	if RB.LAMDONE == 0 then
		RBBuildLAMSettingsMenu()
		EVENT_MANAGER:UnregisterForEvent("RecipeBook", EVENT_PLAYER_ACTIVATED)
	end
end)